...
type Status = 'idle' | 'uploading' | 'uploaded' | 'processing' | 'processed' | 'downloaded';
...
function App() {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      const response = await fetch('https://kjbwkqv4-5000.use.devtunnels.ms/upload', {
        method: 'POST',
        body: formData
      });
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
      <h1 className="text-4xl font-bold mb-8 font-roboto text-gray-800">CREACIÓN DE NOMENCLATURA</h1>
      <div
        {...getRootProps()}
        className={`w-full max-w-2xl h-96 p-12 text-center bg-white rounded-xl shadow-lg transition-all duration-200 
          ${isDragActive ? 'border-2 border-blue-500 bg-blue-50' : 'border-2 border-dashed border-gray-300'}
